package com.blacksnow1002.realmmod.system.profession.alchemy.screen;

import com.blacksnow1002.realmmod.core.registry.ModMenuTypes;
...
public class AlchemyFurnaceMenu extends AbstractContainerMenu {
    ...
    // 槽位索引
    public static final int RECIPE_SLOT = 0;      // 丹方槽
    public static final int IGNORED_SLOT = 1;     // 不使用的槽位
    public static final int DEMON_PILL_SLOT = 2;  // 妖丹槽
    ...
    public AlchemyFurnaceMenu(int id, Inventory playerInv, FriendlyByteBuf data) {
        super(ModMenuTypes.ALCHEMY_FURNACE_MENU.get(), id);
    ...
    // 服務端構造函數
    public AlchemyFurnaceMenu(int id, Inventory playerInv, BlockEntity blockEntity, ItemStackHandler itemHandler) {
        super(ModMenuTypes.ALCHEMY_FURNACE_MENU.get(), id);
    ...
    private void addSlots(Inventory playerInv) {
        // 添加方塊實體的槽位
        // 第一排：丹方和占位槽
        this.addSlot(new SlotItemHandler(itemHandler, RECIPE_SLOT, 98, 17));        // 丹方
        this.addSlot(new SlotItemHandler(itemHandler, IGNORED_SLOT, 134, 17));      // 占位槽（不使用）

        // 第二排：材料槽位
        this.addSlot(new SlotItemHandler(itemHandler, DEMON_PILL_SLOT, 62, 44));    // 妖丹
