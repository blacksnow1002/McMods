...
public class AlchemyToolBlockItem extends BlockItem {

    private final AlchemyToolBlock.Grades grade;
    private final int rank;

    public AlchemyToolBlockItem(Block block, Properties properties, AlchemyToolBlock.Grades grade, int rank) {
     ...
     */
    @Nullable
    public static AlchemyToolBlock.BonusEntry getBonus(ItemStack stack) {
        ...
        if (customData != null) {
            ...
            if (tag.contains("BonusEntry")) {
                ...
                if (index >= 0 && index < AlchemyToolBlock.BonusEntry.values().length) {
                    return AlchemyToolBlock.BonusEntry.values()[index];
                }
            }
        }
    ...
    public static void setRandomBonus(ItemStack stack) {
        ...
        int bonusIndex = random.nextInt(AlchemyToolBlock.BonusEntry.values().length);
     ...
     * 設置指定詞條
     */
    public static void setBonus(ItemStack stack, AlchemyToolBlock.BonusEntry entry) {
        CustomData existingData = stack.get(DataComponents.CUSTOM_DATA);
        CompoundTag tag;
        if (existingData != null) {
            tag = existingData.copyTag();
    ...
    public void appendHoverText(ItemStack stack, @Nullable TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        // 顯示詞條
        AlchemyToolBlock.BonusEntry bonus = getBonus(stack);
        if (bonus != null) {
            ...
            // 如果是天階但沒有詞條，顯示警告
            if (grade == AlchemyToolBlock.Grades.HEAVEN) {
